...
describe('AttributeType/String', () => {
    ...
    it('should store values as strings', () => {
        ...
        (0, chai_1.expect)(record.testString).eq('some value');
        (0, chai_1.expect)(record.get('testString')).eq('some value');
        (0, chai_1.expect)(record.getAttribute('testString')).eq('some value');
        (0, chai_1.expect)(record.getAttributeDynamoValue('testString')).deep.eq({ S: 'some value' });
    });
    it('rejects non-string values', () => {
        (0, chai_1.expect)(record.testString).eq(null);
        // we do this primarily to ensure typing is enforced for queries
        (0, chai_1.expect)(() => { record.testString = 10; }).to.throw();
    });
    describe('default values', () => {
        it('supports default values', () => {
            (0, chai_1.expect)(record.defaultedString).eq('SomeDefault');
        });
        it('ignores default when it has a value from constructor', () => {
            record = setup_tests_spec_1.TestableTable.new({ defaultedString: '123' });
            (0, chai_1.expect)(record.defaultedString).eq('123');
        });
    ...
    describe('additional metadata options', () => {
        it('lowercase', () => {
            ...
            (0, chai_1.expect)(record.lowercaseString).eq('hello world');
        });
        it('uppercase', () => {
            record.uppercaseString = 'Hello World';
            (0, chai_1.expect)(record.uppercaseString).eq('HELLO WORLD');
        });
        it('trim', () => {
            record.trimmedString = ' what ';
            (0, chai_1.expect)(record.trimmedString).eq('what');
        });
    });
});
