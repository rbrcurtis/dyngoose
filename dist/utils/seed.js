...
async function seedTables(input) {
    ...
    const seedFiles = (0, fs_1.readdirSync)(input.seedsDirectory);
    const tableFileSuffix = input.tableFileSuffix.substr(0, 1) === '.' ? input.tableFileSuffix : `.${input.tableFileSuffix}`;
    for (const file of seedFiles) {
        const filePath = (0, path_1.join)(input.seedsDirectory, file);
        let records = [];
        if (file.endsWith('.seed.json')) {
            records = JSON.parse((0, fs_1.readFileSync)(filePath, 'utf8'));
        }
        ...
        if (records.length > 0) {
            ...
            const modelPath = (0, path_1.join)(input.tablesDirectory, `${modelName}${tableFileSuffix}`);
            ...
            for (const ExportedProperty of _.values(tableFileExports)) {
                if ((0, is_1.isDyngooseTableClass)(ExportedProperty)) {
